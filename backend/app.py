...
app.config['SQLALCHEMY_DATABASE_URI'] = os.environ.get('DATABASE_URL', 'postgresql://localhost/leakpeek')
...
def start_analysis():
    ...
    try:
        ...
        db.session.rollback()
        return jsonify({'error': 'Analysis failed to start'}), 500
...
def get_analysis(analysis_id):
    ...
    if analysis.is_expired():
        ...
        db.session.commit()
        return jsonify({'error': 'Analysis expired and has been deleted'}), 410
...
def cleanup_expired_data():
    """Remove expired analyses - run via cron job"""
    try:
        ...
        return jsonify({
            'message': f'Cleaned up {len(expired_analyses)} expired analyses'
        }), 200
    except Exception as e:
        db.session.rollback()
